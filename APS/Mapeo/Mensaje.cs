...
namespace APS.Mapeo
{
    public class Mensaje
    {
        ...
        public static List<Mensaje> ListaMensajesRecibidos(Usuario emisor)
        {
            // Retorna una lista con todos los obejtos de la clase almacenados en la base de datos
            SQLSERVERDB miBD = new SQLSERVERDB(BD_SERVER, BD_NAME);
            List<Mensaje> lista = new List<Mensaje>();

            foreach (object[] tupla in miBD.Select("SELECT ID_Mensaje,emailEmisor, emailReceptor FROM Mensajes WHERE emailReceptor = '" + emisor.Email + "';"))
            {
                String emailE = tupla[1].ToString();
                String emailR = tupla[2].ToString();
                int id = int.Parse(tupla[0].ToString());
